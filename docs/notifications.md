...
`style-light.css` и `style-dark.css` только импортируют динамический
`accent.css` и общий `style-common.css`. `accent.sh` уже записывает в
`accent.css` разрешённые для текущего режима акцент, поверхность и контрастный
цвет; отдельные статические `theme-{light,dark}.css` поэтому не нужны. Геометрия
карточек остаётся в одном общем файле и темы не расходятся.

У контента карточки асимметричный padding `10px 28px 10px 14px`: увеличенный
правый отступ оставляет тексту столько же воздуха, сколько у окон Hyprland.
