...
- повторный `EVIOCGRAB` после hotplug делается только на полностью отпущенной
  клавиатуре: иначе Hyprland видел key-down от физического устройства, key-up
  от виртуального и навсегда оставлял случайную клавишу (`D`, Backspace и т. п.)
  логически зажатой;
- переподключённая клавиатура подхватывается за доли секунды; частый steady-state
  scan безопасен, потому что уже проверенные `event*` не открываются повторно;
- `flush=True` везде.
...
3. **Мониторов два, и на первую попытку отвечают не оба.** Скрипт целится в
   число подключённых коннекторов (`want_displays()` — по `status == connected`
   у карт, имеющих свои i2c-шины) и добирает отставших, а не выходит по первой
   же появившейся подсветке. Пауза между попытками растёт 2→16 с, иначе
   промахи по спящему монитору заливают `dmesg`.

   Повторить `new_device` на неудачной шине **недостаточно**: после провала
   probe клиент остаётся висеть в sysfs без драйвера, и ядро отвечает `EBUSY`
   на занятый адрес. Поэтому сначала `delete_device`, потом создание заново —
   отличать живую шину от мёртвой по наличию `${n}-0037/driver`.

   Частичный успех (один экран из двух) — **не ошибка юнита**: выключенный из
   розетки монитор это норма, а красный `systemctl status` сбивает диагностику
   на ровном месте.
...
скриптах нельзя — и `brightness.sh`, и функция `bright` берут **все** реально
созданные `/sys/class/backlight/ddcci*`.

Второй раз наступили туда же 28.07.2026: подсветка была одна на два монитора.
`DP-2` промолчал на загрузке (`ddcci 2-0037: core device [6e] probe failed: -19`),
а скрипт выходил из ретраев по `have_backlight && break`, то есть по первому
экрану — второй оставался без ddcci до перезагрузки. Симптом: `bright 50`
крутит только один монитор. Проверять `ddcutil detect` (видит оба независимо
от ddcci) против `ls /sys/class/backlight/` — если первый показывает два
дисплея, а второй одну подсветку, дело в привязке, а не в мониторе.

Яркость пишется **во все подсветки последовательно**, а не параллельно в фоне:
у nvidia все i2c-шины сидят под одним локом драйвера, параллельные записи всё
равно выстроятся в очередь, зато вперемешку дают DDC-таймауты и монитор
проглатывает значение. Значение клампится по `max_brightness` каждого экрана.
